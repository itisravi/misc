 ...
 *  * swapcontext, and loops from 0 to 9. Each value is saved in 
 *  i_from_iterator,
 *   * and then swapcontext used to return to the main loop.  The main loop 
 *   prints
 *    * the value and calls swapcontext to swap back into the function. When the 
 *    end
 *     * of the loop is reached, the function exits, and execution switches to 
 *     the
 ...
 *           *    (2) main_context2 : The point in main to which control from 
 *           loop will
 *                *                        flow by switching contexts.
 *                     *    (3) loop_context  : The point in loop to which 
 *                     control from main will
 *                          *                        flow by switching contexts. 
 *                          */
 ...
 *              * The first call will begin loop.  Subsequent calls will switch 
 *              to
